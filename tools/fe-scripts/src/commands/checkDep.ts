...
const findImports = (patterns: string[]) => {
  ...
  patterns.forEach((pattern) => {
    filepaths.push(...globby.sync([pattern, noStoriesGlob, noTestGlob, noTestDirGlob, noStoriesDir], { absolute: true }));
  });
  ...
  filepaths.forEach((filepath) => {
    ...
    try {
      ...
      const patternDImport = new RegExp(/(?<!.*\/\/.*|^[ ]*\*.*|.*RegExp.*)import\((?:["'\s]*\s*)?(.*([@\w_-]+))["'\s].*\);$/, 'mg');
...
function fixMissplacedTypes(name: string, dependencies: StringMap, peerDependencies: StringMap, devDependencies: StringMap) {
...
function fixMissplacedPeers(
  ...
  const missplacedPeers = Object.keys(dependencies).filter((el): el is string => typeof el === 'string' && knownPeers.includes(el));
...
function fixMissingDeps(name: string, dependencies: StringMap, peerDependencies: StringMap, devDependencies: StringMap) {
